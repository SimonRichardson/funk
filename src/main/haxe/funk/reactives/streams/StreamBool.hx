...
class StreamBool {
    ...
    public static function not(stream : Stream<Bool>) : Stream<Bool> return stream.map(function(value) return !value);

    public static function and(stream0 : Stream<Bool>, stream1 : Stream<Bool>) : Stream<Bool> {
        return stream0.zipWith(stream1, function(a : Bool, b : Bool) return a && b);
    }
    ...
    public static function ifThen<T>(stream : Stream<Bool>, block : Stream<T>) : Stream<T> {
        ...
        return StreamTypes.create(function(pulse : Pulse<T>) : Propagation<T> {
            return if(value && (time == pulse.time())) Propagate(pulse);
            else Negate;
        }, [block].toCollection());
    }
